...
s <- 0.7 # survival
F <- 0.8 # site fidelity
Fp <- 0.3 # return probability of temporary emigrants
...
mean.Fp ~ dbeta(1, 1)    # Prior for mean return probability
...
inits <- function(){list(mean.s = runif(1, 0, 1), mean.F = runif(1, 0, 1), mean.Fp = runif(1, 0, 1), 
                         mean.p = runif(1, 0, 1), mean.R = runif(1, 0, 1), mean.Rp = runif(1, 0, 1),
                         mean.f = runif(1, 0, 1), z = ld.init(rCH, first))}
...
parameters <- c("mean.s", "mean.F", "mean.Fp", "mean.p", "mean.R", "mean.Rp", "mean.f")
